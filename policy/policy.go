...
func Start(cfg *Config, storage *storage.RedisClient) *PolicyServer {
	s := &PolicyServer{config: cfg, startedAt: MakeTimestamp()}
	grace := MustParseDuration(cfg.Limits.Grace)
	...
	timeout := MustParseDuration(s.config.ResetInterval)
	s.timeout = int64(timeout / time.Millisecond)

	resetIntv := MustParseDuration(s.config.ResetInterval)
	resetTimer := time.NewTimer(resetIntv)
	Info.Printf("Set policy stats reset every %v", resetIntv)

	refreshIntv := MustParseDuration(s.config.RefreshInterval)
	refreshTimer := time.NewTimer(refreshIntv)
	Info.Printf("Set policy state refresh every %v", refreshIntv)
	...
	Info.Printf("Running with %v policy workers", s.config.Workers)
	return s
}
...
func (s *PolicyServer) resetStats() {
	...
	for key, m := range s.stats {
		...
		if now-bannedAt >= banningTimeout {
			...
			if atomic.CompareAndSwapInt32(&m.Banned, 1, 0) {
				Info.Printf("Ban dropped for %v", key)
				delete(s.stats, key)
				total++
			}
		}
	...
	Info.Printf("Flushed stats for %v IP addresses", total)
}
...
func (s *PolicyServer) refreshState() {
	...
	if err != nil {
		Error.Printf("Failed to get blacklist from backend: %v", err)
	}
	s.whitelist, err = s.storage.GetWhitelist()
	if err != nil {
		Error.Printf("Failed to get whitelist from backend: %v", err)
	}
	Info.Println("Policy state refresh complete")
}
...
func (s *PolicyServer) InBlackList(addy string) bool {
	s.RLock()
	defer s.RUnlock()
	return StringInSlice(addy, s.blacklist)
}

func (s *PolicyServer) InWhiteList(ip string) bool {
	s.RLock()
	defer s.RUnlock()
	return StringInSlice(ip, s.whitelist)
}
...
func (s *PolicyServer) doBan(ip string) {
	...
	Info.Printf("Banned %v with timeout %v on ipset %s", ip, timeout, set)

	_, err := exec.Command(head, args...).Output()
	if err != nil {
		Error.Printf("CMD Error: %s", err)
	}
}
